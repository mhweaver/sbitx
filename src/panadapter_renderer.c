...
{
  cairo_set_source_rgb(gfx, palette[color][0], palette[color][1],
                       palette[color][2]);
}
                                    ...
                                    int span_hz)
{
  ...
  for (int64_t nominal = panadapter_grid_first_hz(view_start - grid_step,
                                                   grid_step);
       nominal <= view_stop + grid_step; nominal += grid_step) {
    const int64_t frequency =
      panadapter_grid_label_hz(nominal, span_hz, grid_step);
    if (frequency <= view_start || frequency >= view_stop)
      continue;
    const int grid_x = panadapter_view_frequency_x(x, width, frequency,
                                                   view_start, span_hz);
...
void panadapter_renderer_draw_labels(cairo_t *gfx, int x, int y, int width,
                                      int grid_height, int64_t view_start,
                                      int span_hz)
{
  ...
  for (int64_t nominal = panadapter_grid_first_hz(view_start - grid_step,
    ...
    const int label_x = panadapter_view_frequency_x(x, width, frequency,
                                                    view_start, span_hz);
    cairo_text_extents_t extents;
    cairo_text_extents(gfx, text, &extents);
    cairo_move_to(gfx, label_x - (int)extents.x_advance / 2,
                  y + grid_height + font->height);
...
void panadapter_renderer_control_rect(int x, int y, int height, int control,
                                      int *control_x,
                                      int *control_y, int *size)
{
...
static void draw_control(cairo_t *gfx, int x, int y, int height, int control,
                         bool enabled)
{
  int control_x, control_y, size;
  panadapter_renderer_control_rect(x, y, height, control,
                                   &control_x, &control_y, &size);
  const double cx = control_x + size / 2.0;
  const double cy = control_y + size / 2.0;

  cairo_save(gfx);
  cairo_set_source_rgba(gfx, palette[COLOR_BACKGROUND][0],
                        palette[COLOR_BACKGROUND][1],
                        palette[COLOR_BACKGROUND][2], 0.82);
  ...
  if (control == PANADAPTER_LEFT || control == PANADAPTER_RIGHT) {
    ...
    cairo_fill(gfx);
  } else if (control == PANADAPTER_ZOOM_OUT
             || control == PANADAPTER_ZOOM_IN) {
...
void panadapter_renderer_draw_controls(cairo_t *gfx, int x, int y, int height,
                                        int span_hz, unsigned enabled)
{
  for (int control = 0; control < PANADAPTER_CONTROL_COUNT; control++)
    draw_control(gfx, x, y, height, control, enabled & (1u << control));

  int control_x, control_y, size;
  panadapter_renderer_control_rect(x, y, height, PANADAPTER_CONTROL_COUNT - 1,
                                   &control_x, &control_y, &size);
  char bandwidth[24];
  ...
  const double text_y = control_y + (size - extents.height) / 2.0
    - extents.y_bearing;
  cairo_set_source_rgba(gfx, palette[COLOR_BACKGROUND][0],
                        palette[COLOR_BACKGROUND][1],
                        palette[COLOR_BACKGROUND][2], 0.82);
                                        ...
                                        int *waterfall)
{
  ...
  cairo_pattern_t *gradient =
    cairo_pattern_create_linear(0, y + grid_height, 0, y);
  cairo_pattern_add_color_stop_rgba(gradient, 0.0,
    palette[WATERFALL_LOW][0], palette[WATERFALL_LOW][1],
    palette[WATERFALL_LOW][2], 0.5 + scope_alpha_plus);
  cairo_pattern_add_color_stop_rgba(gradient, 0.5,
    palette[WATERFALL_MID][0], palette[WATERFALL_MID][1],
    palette[WATERFALL_MID][2], 0.7 + scope_alpha_plus);
  cairo_pattern_add_color_stop_rgba(gradient, 1.0,
    palette[WATERFALL_HIGH][0], palette[WATERFALL_HIGH][1],
    palette[WATERFALL_HIGH][2], 0.9 + scope_alpha_plus);
  ...
  for (int bin = 0; bin < count; bin++) {
    ...
    cairo_line_to(gfx, x + width - bin_x - x_step / 2.0,
                  y + grid_height - spectrum_y);
                                          ...
                                          uint8_t *pixel)
{
  ...
  if (level < 34) {
    ...
    red = palette[WATERFALL_LOW][0]
      + (palette[WATERFALL_MID][0] - palette[WATERFALL_LOW][0]) * mix;
    green = palette[WATERFALL_LOW][1]
      + (palette[WATERFALL_MID][1] - palette[WATERFALL_LOW][1]) * mix;
    blue = palette[WATERFALL_LOW][2]
      + (palette[WATERFALL_MID][2] - palette[WATERFALL_LOW][2]) * mix;
  } else {
    const float mix = (level - 67) / 33.0f;
    red = palette[WATERFALL_MID][0]
      + (palette[WATERFALL_HIGH][0] - palette[WATERFALL_MID][0]) * mix;
    green = palette[WATERFALL_MID][1]
      + (palette[WATERFALL_HIGH][1] - palette[WATERFALL_MID][1]) * mix;
    blue = palette[WATERFALL_MID][2]
      + (palette[WATERFALL_HIGH][2] - palette[WATERFALL_MID][2]) * mix;
  }
  pixel[0] = red * 255;
  pixel[1] = green * 255;
  pixel[2] = blue * 255;
